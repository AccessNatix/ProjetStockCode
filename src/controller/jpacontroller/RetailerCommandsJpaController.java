...
public class RetailerCommandsJpaController implements Serializable {
    ...
    public static RetailerCommandsJpaController getController(){
        if(singleton==null) singleton = new RetailerCommandsJpaController(javax.persistence.Persistence.createEntityManagerFactory("UMLProjectPU"));
        return singleton;
    }

    private RetailerCommandsJpaController(EntityManagerFactory emf) {
        this.emf = emf;
    }
    ...
    public void create(RetailerCommands retailerCommands) {
        ...
        try {
            ...
            if (retailerId != null) {
                retailerId.getRetailerCommandsCollection().add(retailerCommands);
                retailerId = em.merge(retailerId);
            }
            if (commandId != null) {
                commandId.getRetailerCommandsCollection().add(retailerCommands);
    ...
    public void destroy(Integer id) throws NonexistentEntityException {
        ...
        try {
            ...
            Retailer retailerId = retailerCommands.getRetailerId();
            if (retailerId != null) {
                retailerId.getRetailerCommandsCollection().remove(retailerCommands);
                retailerId = em.merge(retailerId);
            }
            Command commandId = retailerCommands.getCommandId();
            if (commandId != null) {
                commandId.getRetailerCommandsCollection().remove(retailerCommands);
                commandId = em.merge(commandId);
            }
    ...
    public List<RetailerCommands> findByRetailerId(int retailerId){
        return getEntityManager()
            .createNamedQuery("RetailerCommands.findByRetailerId")
    ...
    public void reloveAll(){
        ...
        transac.begin();
        em.createNamedQuery("RetailerCommands.removeAll").executeUpdate();
        transac.commit();
        em.close();
    }
